...
/**
 * Markdownテキストを Notion ブロック配列に変換する（簡易版）
 * Notion API のブロック上限 (2000文字) を考慮して分割する
 */
function markdownToBlocks(markdown) {
  ...
  const flushBuffer = () => {
    ...
    for (let i = 0; i < text.length; i += 1900) {
      blocks.push({
        ...
        paragraph: {
          rich_text: [
            { type: "text", text: { content: text.slice(i, i + 1900) } },
          ],
        },
      });
    }
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("### ")) {
      ...
      for (let i = 0; i < itemText.length; i += 1900) {
        blocks.push({
          object: "block",
          type: "bulleted_list_item",
          bulleted_list_item: {
            rich_text: [
              { type: "text", text: { content: itemText.slice(i, i + 1900) } },
            ],
          },
        });
      }
...
/**
 * Notionデータベースに既に登録済みのファイル名一覧を取得する
 */
  ...
  do {
    ...
    for (const page of response.results) {
      ...
      if (prop?.rich_text?.[0]?.plain_text) {
        fileNames.add(prop.rich_text[0].plain_text);
      }
    }
...
async function main() {
  ...
  console.log(`\n=== 同期完了 === 新規登録: ${created}件 / スキップ: ${skipped}件`);
}
